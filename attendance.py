...
# This part is image labels setting start 
        ...
        img=img.resize((1366,790),Image.ADAPTIVE)
        self.photoimg=ImageTk.PhotoImage(img)

        # set image as lable
        f_lb1 = Label(self.root,image=self.photoimg)
        f_lb1.place(x=0,y=0,width=1366,height=790)

        # backgorund image 
        bg1=Image.open(r"img\bg1.jpg")
        bg1=bg1.resize((1366,790),Image.ADAPTIVE)
        self.photobg1=ImageTk.PhotoImage(bg1)       # set image as lable
        bg_img = Label(self.root,image=self.photobg1)
        bg_img.place(x=0,y=40,width=1366,height=790)
        ...
        main_frame.place(x=0,y=60,width=1366,height=540)
    ...
    #     # ===============================Table Sql Data View==========================
        ...
        table_frame.place(x=10,y=130,width=620,height=280)
        ...
        #create table 
        self.attendanceReport_left = ttk.Treeview(table_frame,columns=("ID","Name","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    #     # =========================button section========================
        ...
        btn_frame.place(x=30,y=420,width=500,height=40)

        #Improt button
        save_btn=Button(btn_frame,command=self.importCsv,text="Import CSV",width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        save_btn.grid(row=0,column=0,padx=6,pady=10,sticky=W)

        #Exprot button
        update_btn=Button(btn_frame,command=self.exportCsv,text="Export CSV",width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        update_btn.grid(row=0,column=1,padx=6,pady=8,sticky=W)

        #Update button                    ###*****command=self.action add hoga
        del_btn=Button(btn_frame,text="Update",command=self.action,width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
    ...
    #     # -----------------------------Table Frame-------------------------------------------------
        ...
        table_frame.place(x=10,y=70,width=630,height=400)
        ...
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update stdattendance set std_id=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_name.get(),
                    self.var_time.get(),
                    self.var_date.get(),
    ...
    def action(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                conn = mysql.connector.connect(user='root', password='Dee@Reg45',host='localhost',database='face_recognizer',port=3306)
                mycursor = conn.cursor()
                mycursor.execute("insert into stdattendance values(%s,%s,%s,%s,%s)",(
                self.var_id.get(),
                #self.var_roll.get(),
                self.var_name.get(),
                self.var_time.get(),
        ...
        if messagebox.askyesno("Confirmation","Are you sure you want to save attendance on database?"):
            for i in mydata:
                ...
                udate = i[3]
                uattend = i[4]
                qury = "INSERT INTO stdattendance(std_id, std_name, std_time, std_date, std_attendance) VALUES(%s,%s,%s,%s,%s)"
                mycursor.execute(qury,(uid,uname,utime,udate,uattend))
            conn.commit()
            conn.close()
